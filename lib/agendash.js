...
module.exports = function (agenda, options) {
  ...
  agenda.on("ready", () => {
    ...
    collection.createIndexes(
      ...
      (err) => {
        if (err) {
          console.warn(
            "Agendash indexes might not exist. Performance may decrease."
          );
        }
      }
    );
  ...
  const customCreateJob = (
    ...
    try {
      // @TODO: Need to validate user input.
      const job = agenda.customCreate(jobName, jobData, jobMeta);
      if (jobSchedule && jobRepeatEvery) {
        job.repeatAt(jobSchedule);
        job.repeatEvery(jobRepeatEvery, {
          skipImmediate: true,
  ...
  const customDeleteJobs = (query, callback) => {
    if (!agenda) {
      return callback("Agenda instance is not ready");
    }
    try {
